...
class MCMCSampler:
    def __init__(self, model, img_shape, sample_size, num_classes, cbuffer_size=256):
        ...
        self.buffer = {i: torch.empty(0, *img_shape, device=device) for i in range(num_classes)}  # class-specific buffers

    def _update_buffer(self, imgs, clabel):
        """Update buffer with new images using reservoir sampling"""
        if clabel is None:
            clabel = torch.zeros(imgs.shape[0], dtype=torch.long, device=device)
    ...
    def synthesize_samples(self, clabel=None, steps=60, step_size=0.01, return_img_per_step=False):
        ...
        num_new_images = max(1, self.sample_size // 10)  # e.g., 10% of samples are new
        num_buffer_images = self.sample_size - num_new_images

        inp_imgs = torch.normal(0, 0.01, (num_new_images, *self.img_shape), device=self.model.device) * 2.0 - 1.0
        
        # Sample num_buffer_images from buffers
        if clabel is not None:
            for i, label in enumerate(clabel[:num_buffer_images]):
                if len(self.buffer[label]) > 0:
                    idx = torch.randint(0, len(self.buffer[label]), (1,)).item()
                    inp_imgs = torch.cat([inp_imgs, self.buffer[label][idx:idx+1]], dim=0)
        
        # fill rest with num_new_images random samples
        if inp_imgs.shape[0] < self.sample_size:
            pad_size = self.sample_size - inp_imgs.shape[0]
            padding = torch.normal(0, 0.01, (pad_size, *self.img_shape), device=self.model.device) * 2.0 - 1.0
        ...
        for _ in range(steps):
            ...
            with torch.enable_grad(): # required in validation mode to compute gradients
                energies = self.model(inp_imgs, clabel)
                energy_sum = energies.sum()
                energy_sum.backward()
            gradients = inp_imgs.grad
            ...
            with torch.no_grad():
                inp_imgs -= step_size/2 * gradients
            inp_imgs.grad.zero_()
        ...
        # Update buffer with synthesized samples
        self._update_buffer(inp_imgs.detach(), clabel)
...
def run_generation(args, ckpt_path: Union[str, Path], conditional: bool = False):
    ...
    def gen_imgs(model, clabel=None, step_size=0.001, batch_size=24, num_steps=256):
        model.eval()
    ...
    for label in tqdm.tqdm(conditional_labels):
        ...
        generated_imgs = gen_imgs(model, clabel=clabel if conditional else None, step_size=0.01, batch_size=bs, num_steps=num_steps).cpu()
        ...
        plt.yticks([])
        plt.savefig(f"images_JEM/{'conditional' if conditional else 'unconditional'}_sample_label={label}.png")
    ...
    plt.xticks([])
    plt.yticks([])
    plt.savefig(f"images_JEM/{'conditional' if conditional else 'unconditional'}_samples.png")
...
def run_evaluation(args, ckpt_path: Union[str, Path]):
    ...
    trainer = pl.Trainer(inference_mode=False) #gpus=1 if str(device).startswith("cuda") else 0)
...
if __name__ == '__main__':
    ...
    # 2) Evaluate model
    ckpt_path: str = "JEM_5/lightning_logs/version_0/checkpoints/last_epoch=4-step=1765.ckpt"
